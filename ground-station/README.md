...
`V1.0.2.9` 配套的 E28-2G4M20SX（SX1281）任务通信和 CSV 任务记录。
地面站仍可解码飞控 `V1.0.2.4` 的旧版 30 字节遥测，但方向和飞控日志状态只有
新版 32 字节遥测才提供。
...
远程执行器指令只有在飞控本地执行过 `actuator arm` 后才会被接受；解锁 60 秒后
自动失效。地面站只负责发送请求，最终范围、模式、故障、互锁和超时判定由飞控完成。
...
SD 卡、链路状态、执行器方向与状态、飞控日志状态、射频计数及 RSSI/SNR。
...
build/Debug/GROUND_STATION_BOARD_TEST_V1.1.0.7.hex
```
