...
飞控 `V1.0.5.9` 增加了 IMU 的上电初始化与未通过身份校验时的原始数据读取、CS 静态
...
SPI1 默认切换为 ICM-45686 官方驱动采用的 Mode3，同时保留 Mode0 运行时对照，新增
地址发送后等待时间诊断命令。默认等待时间仍为 0 ms。
...
| `imu spi mode0\|mode3` | 切换 SPI1 的 CPOL/CPHA，默认 Mode3 |
| `imu spi delay 0\|1\|10\|100` | 设置分段读取中发送地址后、接收数据前的等待时间（毫秒） |
