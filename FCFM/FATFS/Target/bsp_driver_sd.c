...
volatile uint32_t g_sd_detect = SD_NOT_PRESENT;
volatile uint32_t g_sd_init_hal = 0xFFFFFFFFU;
volatile uint32_t g_sd_init_error = HAL_SD_ERROR_NONE;
volatile uint32_t g_sd_card_state = HAL_SD_CARD_ERROR;
volatile uint32_t g_sd_hal_state = HAL_SD_STATE_RESET;
volatile uint32_t g_sd_cmd_level = GPIO_PIN_RESET;
volatile uint32_t g_sd_d0_level = GPIO_PIN_RESET;
volatile uint32_t g_sd_clk_level = GPIO_PIN_RESET;
...
__weak uint8_t BSP_SD_Init(void)
{
  ...
  g_sd_detect = BSP_SD_IsDetected();
  g_sd_init_hal = 0xFFFFFFFFU;
  g_sd_init_error = HAL_SD_ERROR_NONE;
  g_sd_card_state = HAL_SD_CARD_ERROR;
  g_sd_hal_state = HAL_SD_GetState(&hsd);

  /* Check if the SD card is plugged in the slot */
  if (g_sd_detect != SD_PRESENT)
  {
    return MSD_ERROR;
  }

  /* Bring the card up safely at 1-bit and 4 MHz, then switch to 4-bit. */
  hsd.Init.BusWide = SDIO_BUS_WIDE_1B;
  hsd.Init.ClockDiv = 10U;

  init_result = HAL_SD_Init(&hsd);
  /* HAL_SD_Init calls HAL_SD_MspInit before issuing card commands.  Capture
     the resulting idle levels even if card initialization failed. */
  g_sd_cmd_level = (uint32_t)HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_2);
  g_sd_d0_level = (uint32_t)HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_8);
  g_sd_clk_level = (uint32_t)HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_12);
  g_sd_init_hal = (uint32_t)init_result;
  g_sd_init_error = HAL_SD_GetError(&hsd);
  g_sd_hal_state = HAL_SD_GetState(&hsd);
  ...
  g_sd_card_state = HAL_SD_GetCardState(&hsd);
  g_sd_init_error = HAL_SD_GetError(&hsd);
  g_sd_hal_state = HAL_SD_GetState(&hsd);

  return (g_sd_card_state == HAL_SD_CARD_TRANSFER) ? MSD_OK : MSD_ERROR;
}
